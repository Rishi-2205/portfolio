...
export default function Projects(){
  return (
    <div>
      <motion.h2 className="text-3xl font-bold mb-6 glow" initial={{opacity:0}} animate={{opacity:1}}>Projects</motion.h2>
      <div className="grid md:grid-cols-3 gap-6">
        {projects.map((p, i) => (
          <motion.a
            ...
            className="card-glass p-5 rounded-2xl border border-slate-700 shadow-lg transform transition-transform"
          >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {p.tech.map(t => <span key={t} className="text-xs px-2 py-1 rounded bg-slate-800/40 border border-slate-700">{t}</span>)}
            </div>
            <div className="mt-4 text-right">
              <span className="text-sm text-neon">View →</span>
            </div>
          </motion.a>
        ))}
      </div>
